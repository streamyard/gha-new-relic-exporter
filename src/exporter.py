...
from custom_parser import (check_env_vars, do_fastcore_decode, do_time,
                           parse_attributes)
...
def get_workflow_run_jobs_by_run_id(page=1, jobs=None):
    ...
    try:
        response = do_fastcore_decode(
            api.actions.list_jobs_for_workflow_run(GHA_RUN_ID, page=page)
        )
...
p_parent = tracer.start_span(
    name=str(GHA_RUN_NAME) + " - run: " + str(GHA_RUN_ID),
    attributes=atts,
